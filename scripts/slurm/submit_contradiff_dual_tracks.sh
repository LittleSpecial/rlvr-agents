...
#   Track B: gymnasium+mujoco online protocol, optional
#
# Example:
#   bash scripts/slurm/submit_contradiff_dual_tracks.sh
#   ENABLE_TRACK_B=1 TRACK_B_CONDA_ENV=$HOME/.conda/envs/rlvr bash scripts/slurm/submit_contradiff_dual_tracks.sh
...
submit_track() {
  ...
  local train_job value_job eval_job
  ...
  submit_and_get_id env \
    CUDA_MODULE="${CUDA_MODULE}" GCC_MODULE="${GCC_MODULE}" NCCL_MODULE="${NCCL_MODULE}" CUDNN_MODULE="${CUDNN_MODULE}" \
    CONDA_ENV="${conda_env}" PYTHON_BIN="${python_bin}" CONTRADIFF_DIR="${CONTRADIFF_DIR}" \
    EXPERIMENT_NAME="${exp_name}" RUN_LOGBASE="${run_logbase}" VALUE_BRANCH="${VALUE_BRANCH}" VALUESEED="${VALUESEED}" \
    DATASET="${DATASET}" EXP_DATASET="${EXP_DATASET}" EXPERT_RATIO="${EXPERT_RATIO}" HORIZON="${HORIZON}" N_DIFFUSION_STEPS="${N_DIFFUSION_STEPS}" \
    SEED="${VALUESEED}" VALUE_STEPS="${VALUE_STEPS}" SAVE_INTERVAL="${SAVE_INTERVAL}" LOG_INTERVAL="${LOG_INTERVAL}" \
    LEARNING_RATE="${LEARNING_RATE}" VALUE_BATCH_SIZE="${VALUE_BATCH_SIZE}" VALUE_RENDERER="${VALUE_RENDERER}" \
    sbatch --dependency=afterok:"${train_job}" "${SUBMIT_VALUE_SCRIPT}"
  value_job="${SUBMITTED_JOB_ID:-}"
  if [ -z "${value_job}" ]; then
    echo "[ERR] failed to submit value job for ${exp_name}" >&2
    return 1
  fi
  ...
  echo "[${track}] exp=${exp_name} train_job=${train_job} value_job=${value_job} eval_job=${eval_job}"
}
